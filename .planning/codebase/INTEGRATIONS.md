...
- SDK/Client: `@supabase/supabase-js` — browser maps/client `apps/web/src/lib/supabase.ts`; identity server client `apps/web/src/lib/service-role-supabase.ts` (`identitySupabase`, `persistSession: false`); tenant BFF `apps/web/src/lib/tenant-supabase.ts` (anon key + minted JWT); package client `packages/database/src/index.ts` (lazy Proxy); scripts `scripts/run-migration.js`, `scripts/fix-rls.js`, `scripts/apply-rls-fix.js`.
- Auth: anon key via `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY`. Required `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_JWT_SECRET` in `apps/web/src/lib/env.ts`. Identity (authorize, signup, invites) uses service_role; tenant leave/notification BFF must not. Local stack: `supabase/config.toml` (`project_id = "timeoff"`). Remote typegen: `npm run supabase:gen:types` writes to `packages/database/src/types.ts` using `--project-id lvscrngetawbztaiadpr`.
- Used by: identity — `apps/web/src/lib/auth.ts`, `apps/web/src/app/api/auth/signup/route.ts`, `apps/web/src/app/api/auth/invites/*`; diagnostics — `apps/web/src/app/api/test-connection/route.ts` (env SET/NOT SET only); browser provider — `apps/web/src/providers/database-provider.tsx`; repositories under `packages/database/src/modules/*/repository.ts`.
