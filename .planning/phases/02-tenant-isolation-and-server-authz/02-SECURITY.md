...
UAT: complete (5/5 pass). ASVS L1 (presence). `block_on: high`. One blocking open threat: leave-list PostgREST embeds still return `users(*)` including password hashes (T-02-12 / CR-01).
...
| T-02-08 | Elevation of Privilege | PATCH `/api/leave-requests/[id]` | high | mitigate | `bindLeaveApprover` from session; Zod action enum | closed |
| T-02-09 | Tampering | POST `/api/leave-requests/bulk` | high | mitigate | Session gate; session `approver_id`; RLS denies cross-tenant | closed |
...
| T-02-12 | Information Disclosure | UserRepository / leave join | high | mitigate | `USER_DOMAIN_COLUMNS` omit password on findById/findAll/getTeamMembers; **leave-request joins still `users(*)` so team/all GET JSON includes hashes** | open |
...
| T-02-12 | `packages/database/src/modules/users/repository.ts` `USER_DOMAIN_COLUMNS`; still `users!leave_requests_user_id_fkey(*)` in `leave-requests/repository.ts` (lines 21, 55, 77, 99, 284). `02-REVIEW.md` CR-01. |
...
**Approval:** pending — close T-02-12 (omit `password` from leave-request user embeds) then re-run `/gsd-secure-phase 2`
